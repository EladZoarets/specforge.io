...
def build_user_prompt(story: JiraStory, phase1: Phase1Result) -> str:
    ...
    return (
        ...
        f"- Quality ({phase1.quality.score:.1f}): {phase1.quality.rationale}\n"
        f"- Ambiguity ({phase1.ambiguity.score:.1f}): {phase1.ambiguity.rationale}\n"
        f"- Complexity ({phase1.complexity.score:.1f}): {phase1.complexity.rationale}\n"
        f"{suggestions_block}\n"
...
def _format_suggestions(phase1: Phase1Result) -> str:
    ...
    for score in (phase1.quality, phase1.ambiguity, phase1.complexity):
        for suggestion in score.suggestions:
            lines.append(f"- [{score.agent_name}] {suggestion}")
    if not lines:
